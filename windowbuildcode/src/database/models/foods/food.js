...
const dbManager = require('../../database-manager.cjs');
      ...
      if (foodData.image) {
        try {
          ...
          if (oldFood.success && oldFood.data.image) {
            const oldImagePath = path.join(uploadsDir, oldFood.data.image);
...
export function deleteFoodImage(foodId) {
  try {
    ...
    if (food.success && food.data.image) {
      const imagePath = path.join(uploadsDir, food.data.image);
