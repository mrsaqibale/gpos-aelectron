...
// Enhanced database path resolver for both development and production
export function getDatabasePath(relativePath = 'pos.db') {
  try {
...
export function getUploadsPath(subPath = '') {
  try {
    const isDev = !__dirname.includes('app.asar') && fs.existsSync(path.join(__dirname, 'pos.db'));
