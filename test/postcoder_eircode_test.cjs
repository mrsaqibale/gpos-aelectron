...
// Usage: node test/postcoder_eircode_test.cjs [query]
...
async function run() {
  ...
  const url = `${POSTCODER_BASE_URL}/${POSTCODER_API_KEY}/address/IE/${encoded}?position=true`;
  ...
  try {
    ...
    const withGeo = json.filter((i) =>
      (Object.prototype.hasOwnProperty.call(i, 'latitude') || Object.prototype.hasOwnProperty.call(i, 'longitude')) &&
