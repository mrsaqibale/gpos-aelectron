...
function getDatabasePath() {
  try {
    // In development, DB lives under src/database
    const devDb = path.join(__dirname, '../src/database/pos.db');
    const isDev = !__dirname.includes('app.asar') && fs.existsSync(devDb);
    if (isDev) return devDb;

    // Production: bundled under resources/database or copied to AppData
    const possiblePaths = [
    ...
    const appDataDir = path.join(process.env.APPDATA || process.env.HOME || '', 'GPOS System', 'database');
    if (!fs.existsSync(appDataDir)) fs.mkdirSync(appDataDir, { recursive: true });
    const finalPath = path.join(appDataDir, 'pos.db');

    for (const candidate of possiblePaths) {
      try { if (candidate && fs.existsSync(candidate)) { fs.copyFileSync(candidate, finalPath); break; } } catch (_) {}
    }
