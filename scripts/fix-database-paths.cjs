...
module.exports = function afterAllArtifactBuild(context) {
  console.log('Running database path fix script...');

  // Get the app output directory
  const appOutDir = context.appOutDir || path.join(__dirname, '../dist/linux-unpacked');
  
  // Copy database files to the correct location in the built app
  const sourceDbPath = path.join(__dirname, '../src/database');
  const targetDbPath = path.join(appOutDir, 'database');
  ...
  if (fs.existsSync(sourceDbPath)) {
    ...
    dbFiles.forEach(file => {
      ...
      if (fs.existsSync(sourceFile)) {
        ...
        console.log(`Copied ${file} to built app directory`);
      }
    });
    ...
    if (fs.existsSync(sourceMigrationsPath)) {
      ...
      console.log('Copied migrations directory to built app directory');
    }
    ...
    if (fs.existsSync(sourceModelsPath)) {
      ...
      console.log('Copied models directory to built app directory');
    }
  }

  console.log('Database path fix script completed successfully!');
}; 